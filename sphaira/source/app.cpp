    ...
    if (App::GetReplaceHbmenuEnable() != enable) {
        ...
        if (!enable) {
            ...
            if (R_SUCCEEDED(nro_get_nacp("/hbmenu.nro", hbmenu_nacp))) {
                if (std::strcmp(hbmenu_nacp.lang[0].name, APP_NAME)) {
                    return;
                }
            }
            ...
            App::Push<ui::OptionBox>(
                ...
                "Back"_i18n, "Restore"_i18n, 1, [hbmenu_nacp](auto op_index){
                    ...
                    fs::FsPath hats_path = "/switch/mm-tools/mm-tools.nro";
                    Result rc;

                    // first, try and backup hats-tools, its not super important if this fails.
                    rc = nro_get_nacp(hats_path, hats_nacp);
                    if (R_FAILED(rc) || std::strcmp(hats_nacp.lang[0].name, APP_NAME)) {
                        hats_path = "/switch/mm-tools.nro";
                        rc = nro_get_nacp(hats_path, hats_nacp);
                    }

                    if (R_SUCCEEDED(rc) && !std::strcmp(hats_nacp.lang[0].name, APP_NAME)) {
                        ...
                        hats_path = "/switch/mm-tools/mm-tools.nro";
                        g_app->m_fs->CreateDirectoryRecursively("/switch/mm-tools/");
                    ...
                    if (R_FAILED(rc = g_app->m_fs->copy_entire_file("/hbmenu.nro", "/switch/hbmenu.nro")))  {
                        ...
                        if (R_FAILED(rc = g_app->m_fs->copy_entire_file("/hbmenu.nro", hats_path))) {
                            ...
                            App::Push<ui::OptionBox>(
                                "Failed to restore hbmenu, using " APP_NAME " instead"_i18n,
                                "OK"_i18n
                            );
                        }
                        return;
                    ...
                    if (IsHbmenu()) {
                        App::Push<ui::OptionBox>(
                            "Restored hbmenu, closing " APP_NAME ""_i18n,
                            "OK"_i18n, [](auto) {
                                App::Exit();
                            }
                        );
...
Result App::SetDefaultBackgroundMusic(fs::Fs* fs, const fs::FsPath& path) {
    constexpr const char* base_path = "/config/mm-tools/themes/default_music.";
...
App::App(const char* argv0) {
    ...
    // This ensures the HATS installer payload has a valid install mode from first run
    {
        if (!m_fs->FileExists(CONFIG_PATH)) {
            ...
            m_fs->CreateDirectoryRecursively("/config/mm-tools");
            ...
            ini_puts("pack", "pack_url", "file:///config/mm-tools/releases.json", CONFIG_PATH);
            ini_puts("installer", "install_mode", "overwrite", CONFIG_PATH);     // Default install mode
            ini_puts("installer", "payload", "/switch/mm-tools/hats-installer.bin", CONFIG_PATH);
            ini_puts("installer", "staging_path", "/config/mm-tools/hats-staging", CONFIG_PATH);
            ini_puts("firmware", "firmware_url", "https://api.github.com/repos/sthetix/NXFW/releases", CONFIG_PATH);
    ...
    utils::Async async_init([this](){
...

        {
            ...
            m_fs->CreateDirectoryRecursively("/config/mm-tools");
            m_fs->CreateDirectory("/config/mm-tools/themes");
        }
...

        {
            ...
            m_fs->DeleteDirectoryRecursively("/switch/mm-tools/cache/themezer"); // themezer icon/json cache.
            m_fs->DeleteFile("/switch/mm-tools/cache/cache.json"); // old etag cache.
        }
...

        {
            ...
            if (loader_info_size) {
                if (loader_info_size >= std::strlen(APP_NAME) && !std::memcmp(envGetLoaderInfo(), APP_NAME, std::strlen(APP_NAME))) {
                    log_write("launching from " APP_NAME " created forwarder\n");
...
void App::DisplayAdvancedOptions(bool left_side) {
    ...
    }, "Logs to /config/mm-tools/log.txt"_i18n);
    ...
    }, App::GetThemeIndex(), "Customise the look of " APP_NAME " by changing the theme"_i18n);
    ...
    options->Add<ui::SidebarEntryArray>("Install option"_i18n, install_mode_items, [install_mode_index](s64& index_out){
        ...
        // Save to HATS Tools universal config at sd:/config/mm-tools/config.ini
        ...
        App::Notify(notify_msg);
            ...
            "This writes to sd:/config/mm-tools/config.ini\n"
    ...
    log_write("starting to exit\n");
    {
        ...
        // backup hbmenu if it is not sphaira
        {
            ...
            if (App::GetReplaceHbmenuEnable() && !IsHbmenu()) {
                ...
                if (R_SUCCEEDED(rc = nro_get_nacp("/hbmenu.nro", hbmenu_nacp)) && std::strcmp(hbmenu_nacp.lang[0].name, APP_NAME)) {
