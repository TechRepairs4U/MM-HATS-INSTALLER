...
constexpr fs::FsPath CACHE_PATH{"/switch/mm-tools/cache/sphaira_latest.json"};

// paths where sphaira can be installed, used when updating
constexpr const fs::FsPath SPHAIRA_PATHS[]{
    "/hbmenu.nro",
    "/switch/mm-tools.nro",
    "/switch/mm-tools/mm-tools.nro",
};
...
const MiscMenuEntry MISC_MENU_ENTRIES[] = {
    ...
    { .name = "GitHub", .title = "GitHub", .func = MiscMenuFuncGenerator<ui::menu::gh::Menu>, .flag = MiscMenuFlag_Shortcut, .info =
        ...
        "View all MM HATS INSTALLER and Firmware releases.\n"
        "Custom entries can be added to /config/mm-tools/github" },
};

auto InstallUpdate(ProgressBox* pbox, const std::string url, const std::string version) -> Result {
    static fs::FsPath zip_out{"/switch/mm-tools/cache/update.zip"};
    ...
    if (!pbox->ShouldExit()) {
        ...
        if (found_exe) {
            for (auto& path : SPHAIRA_PATHS) {
                ...
                // check that this is really MM HATS INSTALLER.
                log_write("[UPD] checking nacp\n");
                NacpStruct nacp;
                if (R_SUCCEEDED(nro_get_nacp(path, nacp)) && !std::strcmp(nacp.lang[0].name, APP_NAME)) {
...
MainMenu::MainMenu() {
    ...
    this->SetActions(
        ...
        std::make_pair(Button::Y, Action{"Options"_i18n, [this](){
            ...
            // build menus info.
            std::string menus_info = "Launch one of " APP_NAME " menus:\n"_i18n;
            for (auto& e : GetMenuMenuEntries()) {
