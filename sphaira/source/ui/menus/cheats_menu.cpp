...
constexpr const char* TOKEN_PATH = "/config/mm-tools/cheatslips_token.json";

// nx-cheats-db local database paths (optional cache)
constexpr const char* NX_DB_PATH = "/config/mm-tools/cheats-db";
...
constexpr const char* CHEAT_METADATA_CACHE_PATH = "/config/mm-tools/cheat-metadata.json";
...
auto SaveCheatMetadataCacheUnlocked(const std::unordered_map<u64, CachedCheatMetadata>& entries) -> bool {
    fs::FsNativeSd fs;
    fs.CreateDirectoryRecursively("/config/mm-tools");
...
// Clear cached cheats database from /config/mm-tools/cheats-db
