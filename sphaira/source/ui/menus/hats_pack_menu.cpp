...
constexpr const char* CACHE_PATH = "/switch/mm-tools/cache/hats";
constexpr const char* RELEASES_CACHE = "/switch/mm-tools/cache/hats/releases.json";
constexpr const char* DOWNLOAD_TEMP = "/switch/mm-tools/cache/hats/download.zip";
constexpr const char* PREFERRED_PACK_ASSET = "HATS.22.1.0.MM.zip";

// HATS paths are now configurable via config.ini under [hats] section
// Defaults: installer_payload=/switch/mm-tools/hats-installer.bin
    ...
    JSON_OBJ_ITR(
        ...
        case cexprHash("assets"): {
            if (yyjson_is_arr(val)) {
                ...
                const char* fallback_url = nullptr;
                u64 fallback_size = 0;
                ...
                yyjson_arr_foreach(val, idx, max, hit) {
                    ...
                    if (name_val && url_val) {
                        ...
                        if (name_str && strstr(name_str, ".zip")) {
                            ...
                            const auto asset_size = size_val ? yyjson_get_uint(size_val) : 0;

                            if (!fallback_name) {
                                fallback_name = name_str;
                                fallback_url = url_str;
                                fallback_size = asset_size;
                            }

                            if (strcmp(name_str, PREFERRED_PACK_ASSET) == 0) {
                                e.asset_name = name_str;
                                e.download_url = url_str;
                                e.size = asset_size;
                                break;
                            }
                        }
                ...
                if (e.asset_name.empty() && fallback_name) {
                    e.asset_name = fallback_name;
                    e.download_url = fallback_url;
                    e.size = fallback_size;
                }
            }
        } break;
    );
