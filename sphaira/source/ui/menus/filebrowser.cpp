...
, m_arg_path{arg_path} {
    ...
    auto callback = this->Add<SidebarEntryCallback>("Create", [this, file_name](){
        ...
        if (!m_db_indexs.empty()) {
            fs::FsNativeSd().read_entire_file("/config/mm-tools/logo/rom/NintendoLogo.png", config.logo);
            fs::FsNativeSd().read_entire_file("/config/mm-tools/logo/rom/StartupMovie.gif", config.gif);
        }
...
void FsView::ZipFiles(fs::FsPath zip_out) {
    ...
    App::Push<ui::ProgressBox>(0, "Compressing "_i18n, "", [this, zip_out, targets](auto pbox) -> Result {
        ...
        ON_SCOPE_EXIT(zipClose(zfile, APP_NAME " v" APP_DISPLAY_VERSION));
...
void Base::LoadAssocEntries() {
    if (m_options & FsOption_LoadAssoc) {
        ...
        LoadAssocEntriesPath("/config/mm-tools/assoc/");
    }
}
